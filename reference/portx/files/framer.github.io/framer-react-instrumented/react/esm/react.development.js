...
function createElement(type, config, children) {
  ...
  return ReactElement(type, key, props, getOwner(), (trackActualOwner ? Error('react-stack-top-frame') : unknownOwnerDebugStack), (trackActualOwner ? createTask(getTaskName(type)) : unknownOwnerDebugTask));
}
function cloneAndReplaceKey(oldElement, newKey) {
  var clonedElement = ReactElement(oldElement.type, newKey, oldElement.props, oldElement._owner, oldElement._debugStack, oldElement._debugTask);
  {
...
function cloneElement(element, config, children) {
  ...
  var clonedElement = ReactElement(element.type, key, props, owner, element._debugStack, element._debugTask);
...
function lazyInitializer(payload) {
  if (payload._status === Uninitialized) {
    ...
    {
      var _ioInfo3 = payload._ioInfo;
      if (_ioInfo3 != null) {
        // Stash the thenable for introspection of the value later.
        // $FlowFixMe[cannot-write]
        _ioInfo3.value = thenable;
        var displayName = thenable.displayName;
        if (typeof displayName === 'string') {
          // $FlowFixMe[cannot-write]
          _ioInfo3.name = displayName;
        }
      }
    }
    if (payload._status === Uninitialized) {
...
export { REACT_ACTIVITY_TYPE as Activity, Children, Component, REACT_FRAGMENT_TYPE as Fragment, REACT_PROFILER_TYPE as Profiler, PureComponent, REACT_STRICT_MODE_TYPE as StrictMode, REACT_SUSPENSE_TYPE as Suspense, ReactSharedInternals as __CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE, ReactCompilerRuntime as __COMPILER_RUNTIME, act, cache, cacheSignal, captureOwnerStack, cloneElement, createContext, createElement, createRef, experimental_useOptimistic, forwardRef, isValidElement, lazy, memo, startTransition, REACT_SUSPENSE_LIST_TYPE as unstable_SuspenseList, REACT_VIEW_TRANSITION_TYPE as unstable_ViewTransition, addTransitionType as unstable_addTransitionType, getCacheForType as unstable_getCacheForType, postpone as unstable_postpone, startGestureTransition as unstable_startGestureTransition, useCacheRefresh as unstable_useCacheRefresh, use, useActionState, useCallback, useContext, useDebugValue, useDeferredValue, useEffect, useEffectEvent, useId, useImperativeHandle, useInsertionEffect, useLayoutEffect, useMemo, useOptimistic, useReducer, useRef, useState, useSyncExternalStore, useTransition, ReactVersion as version };
export default { Activity: REACT_ACTIVITY_TYPE, Children, Component, Fragment: REACT_FRAGMENT_TYPE, Profiler: REACT_PROFILER_TYPE, PureComponent, StrictMode: REACT_STRICT_MODE_TYPE, Suspense: REACT_SUSPENSE_TYPE, __CLIENT_INTERNALS_DO_NOT_USE_OR_WARN_USERS_THEY_CANNOT_UPGRADE: ReactSharedInternals, __COMPILER_RUNTIME: ReactCompilerRuntime, act, cache, cacheSignal, captureOwnerStack, cloneElement, createContext, createElement, createRef, experimental_useOptimistic, forwardRef, isValidElement, lazy, memo, startTransition, unstable_SuspenseList: REACT_SUSPENSE_LIST_TYPE, unstable_ViewTransition: REACT_VIEW_TRANSITION_TYPE, unstable_addTransitionType: addTransitionType, unstable_getCacheForType: getCacheForType, unstable_postpone: postpone, unstable_startGestureTransition: startGestureTransition, unstable_useCacheRefresh: useCacheRefresh, use, useActionState, useCallback, useContext, useDebugValue, useDeferredValue, useEffect, useEffectEvent, useId, useImperativeHandle, useInsertionEffect, useLayoutEffect, useMemo, useOptimistic, useReducer, useRef, useState, useSyncExternalStore, useTransition, version: ReactVersion };
